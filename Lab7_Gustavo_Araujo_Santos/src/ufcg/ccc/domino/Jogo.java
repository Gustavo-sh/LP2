...
public class Jogo {
	...
	public void rodada() throws EstrategiaInvalidaException, JogadaInvalidaException {
		...
		if (jogador1.getNumPecas() == 0) {
			...
			if (this.jogador1.getUltimaJogada().isCarrocao()) {
		...
		if (jogador2.getNumPecas() == 0) {
			...
			if (this.jogador2.getUltimaJogada().isCarrocao()) {
				this.pontuacaoVencedor = 2;
			}
			if (this.pontuacaoVencedor == 0) {
				this.pontuacaoVencedor += 1;
		...
		/* Se ambos passarem, tiverem o mesmo número de peças na mão e
		 * a mesma quantidade de pontos das peças, é empate.
		 * O principal critério de desempate é o número de peças
		 * na mão, em seguida é que se analisa o somatório dos pontos.
		 */
		if (jogadaJ1.getTipo() == TipoJogada.PASSA && jogadaJ2.getTipo() == TipoJogada.PASSA) {
			...
			if (this.jogador1.getSomatorioPecas() < this.jogador2.getSomatorioPecas()) {
				this.vencedor = this.jogador1.getNome();
				this.pontuacaoVencedor += 1;
				this.finalizado = true;
				return;
			} else if (this.jogador2.getSomatorioPecas() < this.jogador1.getSomatorioPecas()) {
				this.vencedor = this.jogador2.getNome();
				this.pontuacaoVencedor += 1;
				this.finalizado = true;
				return;
