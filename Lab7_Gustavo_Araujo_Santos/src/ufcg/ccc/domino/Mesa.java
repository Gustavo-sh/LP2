...
public class Mesa implements VisaoDaMesa {
	...
	/**
	 * Verifica se a ultima jogada ocasionou Lá e Lô. 
	 * Usado para calculo de pontuação.
	 * 
	 * @return true caso tenha side. false, caso não.
	 */
	public boolean isLaELo() {
		if (this.pecas.getFirst().getNumEsquerdo() == this.pecas.getLast().getNumDireito()) {
			return true;
		}
		return false;
	}
	...
	/**
	 * Verifica se a ultima jogada ocasionou Lá e Lô de carroção. 
	 * Usado para calculo de pontuação.
	 * 
	 * @return true caso tenha side. false, caso não.
	 */
	public boolean isLaELoCarrocao(Jogada jogada) {
		if (this.pecas.getFirst().getNumEsquerdo() == this.pecas.getLast().getNumDireito()) {
			if (jogada.isCarrocao())
			return true;
		}
		return false;
	}
